...
def get_minibatch(file_name, batch_size, shuffle, with_pauses=False):
    ...
    if len(dataset) < batch_size:
        print ("WARNING: Not enough samples in "+ file_name + ". Reduce mini-batch size to", len(dataset), "or use a dataset with at least", MINIBATCH_SIZE * data.MAX_SEQUENCE_LEN, " words.")
    ...
    for epoch in range(starting_epoch, MAX_EPOCHS):
        ...
        for X, Y in get_minibatch(data.TRAIN_FILE, MINIBATCH_SIZE, shuffle=True):
            ...
            if iteration % 100 == 0:
                ...
                sys.stdout.flush()
        print ("Total number of training labels: ", total_num_output_samples)
        ...
        if (total_num_output_samples != 0): 
            ...
            if ppl <= best_ppl:
                ...
                print ("Finished!")
                print ("Best validation perplexity was " + best_ppl)
                break
